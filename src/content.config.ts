...
const EXPECTED_PUBLIC_POST_COUNT = 11;
const postSourcePrefix = "src/content/posts/";
const publicPostPatterns = manifest.articles.map((article) => {
	const expectedMarkdownPath = `${postSourcePrefix}${article.slug}/index.md`;
	if (article.paths.markdown !== expectedMarkdownPath) {
		throw new Error(
			`Manifest path mismatch for ${article.slug}: expected ${expectedMarkdownPath}, received ${article.paths.markdown}`,
		);
	}
...
if (
	publicPostPatterns.length !== EXPECTED_PUBLIC_POST_COUNT ||
	new Set(publicPostPatterns).size !== EXPECTED_PUBLIC_POST_COUNT
) {
	throw new Error(
		`The public posts collection requires exactly ${EXPECTED_PUBLIC_POST_COUNT} unique manifest entries`,
	);
}
