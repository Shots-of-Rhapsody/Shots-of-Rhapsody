...
export function getPostAssetBasePath(entry: CollectionEntry<"posts">): string {
	const sourcePath = entry.filePath?.replace(/\\/g, "/");
	const sourcePrefix = "src/content/posts/";
	if (!sourcePath?.startsWith(sourcePrefix)) {
		throw new Error(
			`Post ${entry.id} does not have a file path beneath ${sourcePrefix}`,
		);
	}
	const sourceDirectory = sourcePath.slice(0, sourcePath.lastIndexOf("/"));
	return sourceDirectory.replace(/^src\//, "");
}
export type Tag = {
	name: string;
	count: number;
